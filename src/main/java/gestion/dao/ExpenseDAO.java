...
public class ExpenseDAO {
  ...
  public List<Expense> getPendingRefunds(){
      return expenses.stream().filter(e -> !((RefundableExpense) e).isRefunded()).toList();
  }

  public double getTotalRecurringAmount(){
      return expenses.stream().filter(e -> e instanceof RecurringExpense).mapToDouble(Expense::getAmount).sum();
  }
